...
async function createIndexes() {
  ...
  try {
    ...
    // ==================== PASSWORD RESETS ====================
    var passwordResets = database.collection('password_resets');
    await passwordResets.createIndex({ token: 1 }, { unique: true, name: 'idx_password_reset_token' });
    await passwordResets.createIndex({ userId: 1, createdAt: -1 }, { name: 'idx_password_reset_user' });
    await passwordResets.createIndex({ expiresAt: 1 }, { expireAfterSeconds: 0, name: 'idx_password_reset_ttl' });

    // ==================== ADMIN AUDIT LOG ====================
    var adminAuditLog = database.collection('admin_audit_log');
    await adminAuditLog.createIndex({ adminUserId: 1, timestamp: -1 }, { name: 'idx_admin_audit_admin' });
    ...
    await rateLimits.createIndex({ expiresAt: 1 }, { expireAfterSeconds: 0, name: 'idx_rate_limits_ttl' });

    // ==================== REFRESH TOKENS (CRIT-04) ====================
    var refreshTokens = database.collection('refresh_tokens');
    await refreshTokens.createIndex({ token: 1 }, { unique: true, name: 'idx_refresh_token' });
    await refreshTokens.createIndex({ userId: 1 }, { name: 'idx_refresh_token_user' });
    await refreshTokens.createIndex({ expiresAt: 1 }, { expireAfterSeconds: 0, name: 'idx_refresh_token_ttl' });

    // ==================== IDEMPOTENCY KEYS (SYS-01) ====================
    ...
    await notifications.createIndex({ userId: 1, read: 1 }, { name: 'idx_notifications_user_read' });
